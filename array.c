...
#define PUTIN(n)\
{\
	if(write(arrays, n, strlen((const char*)n)) != strlen((const char*)n)) { doerror(); }; \
	if(write(arrays, "\n", 1) != 1) { doerror(); }; \
}
...
int data(char command) {
	...
	if(command == LOAD) {
        ...
        //array_ixate through the data
				for(array_ix = 0;array_ix < arraysize;array_ix++)	{
					if(parsed[ix])	{
...
unsigned char*getvalue(struct ray*toget, unsigned long index) { 	
	...
	if(index >= toget->nmemb)	{
		error = ERR_OOB;
		return(0);
	}

	if(toget->values[index] == 0) {
		error = ERR_VOID;
		return(0);	
	}
...
int main(int argc, unsigned char*argv[]) {
  ...
  DBASE = (char*)malloc(sizeof(char) * 10);
  strcpy(DBASE, ".array");
	...
	while(argc>1) {
		...
		if(!strcmp((const char*)param, "--flush")) {
			...
			printf("  --file name  Use file name\n");
		...
		} else if(!strcmp((const char*)param, "--file")) {
      DBASE = argv[2];
      argv++;
      argc--;
